...
//----------------== implement a Trait with custom Error
pub struct Record {
  pub key: String,
  pub value: String,
}
...
pub enum ParsingError {
  #[error("{0} is not formatted like a valid record")]
  MulformRecord(String),
}
/*implement built-in trait for fallible conversion
pub trait TryFrom<T>: Sized {
  type Error;//returned type
...
impl TryFrom<&str> for Record {
  ...
  fn try_from(value: &str) -> Result<Self, Self::Error> {
    if !value.contains("=") {
      return Err(ParsingError::MulformRecord(value.into()));
    }
    let split: Vec<String> = value.split("=").map(str::trim).map(str::to_owned).collect();
    if split.len() > 2 {
      return Err(ParsingError::MulformRecord(value.into()));
    }
    let key = split.first().expect("Could not extract the record key");
    let value = split.get(1).expect("Could not extract the record value");
    Ok(Self {
      key: key.into(),
      value: value.into(),
    })
...
Err(err) => match err {
    ParsingError::MulformRecord(err_one) => handle_err_one(err_one),
  }
  */
//--------------------==
//--------------------==
